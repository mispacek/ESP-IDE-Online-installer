const connect = async button => {
  import('./install-dialog-C_XP5tpk.js');
  let port;
  try {
    port = await navigator.serial.requestPort();
  } catch (err) {
    if (err.name === "NotFoundError") {
      import('./index-Bw_teB3E.js').then(mod => mod.openNoPortPickedDialog(() => connect(button)));
      return;
    }
...
class InstallButton extends HTMLElement {
  connectedCallback() {
    ...
    if (!InstallButton.isSupported || !InstallButton.isAllowed) {
      ...
      this.renderRoot.innerHTML = !InstallButton.isAllowed ? "<slot name='not-allowed'>Zařízení ESP můžete flashovat pouze na webech s HTTPS nebo na localhost.</slot>" : "<slot name='unsupported'>Váš prohlížeč nepodporuje Webserial. Použijte prosím Google Chrome nebo Microsoft Edge.</slot>";
      return;
    }
